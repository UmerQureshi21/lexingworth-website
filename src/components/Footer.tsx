...
export default function Footer() {
  ...
  return (
    <footer className="bg-[var(--color-primary)] text-white pt-10 pb-6 border-t border-white/20">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="border-t border-white/10 pt-4">
          <p className="text-xs text-gray-400 text-center md:text-left">
            <span
              onClick={() => setLaunch((n) => n + 1)}
              className="cursor-pointer hover:text-white transition-colors duration-300"
            >
              ©
            </span>{" "}
            {new Date().getFullYear()} Lexingworth Capital. All rights
            reserved.
          </p>
        </div>
      </div>
      ...
      {launch > 0 && (
        <span
          key={launch}
          onAnimationEnd={() => setLaunch(0)}
          className="fixed left-0 top-0 z-[9999] text-4xl pointer-events-none animate-rocket"
        >
          🚀
        </span>
      )}
